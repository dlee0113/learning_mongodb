...
if (isset($_POST['data'])) {
	foreach ($_POST['data'] as $id => $data) {
		if (isset($data['del']) && $data['del'] == 'Y') {
			$member->remove($id);
		} elseif (isset($data['update']) && $data['update'] == 'Y') {
			$member->remove($id);
			$member->adminAdd($data);
		}
	}
	header('Location: ' . HOME_URL . '?page=manage');
	exit;
}
...
?>
	...
	<div class="product-list">
		...
		<?php if (!isset($_POST['change'])) : ?>
			No Changes!
		<?php else : ?>
			<!-- // *** rewrite using $member->getHistoryById() -->
			<?php foreach ($_POST['change'] as $id => $value) : ?>
				...
				<?php if ($value == 'history') : ?>
					<?php $history = $member->getHistoryById($id); ?>
					...
					<?php foreach ($history as $purchase) : ?>
						...
						<td><?php echo $purchase['transaction'] ?></td>	
						...
						<td><?php echo $purchase['special'] ?></td>	
						</tr>
					<?php endforeach; ?>
					</table>
					<p>
